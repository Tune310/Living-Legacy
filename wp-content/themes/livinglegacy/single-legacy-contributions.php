 ...
 if(!empty($media_files)){
	foreach($media_files as $media_check)
	{
		if(!empty($media_check)){
			if(!empty($media_check['url']) and $media_check['type']=='image'){
...
Created by <?php echo get_author_fullname( $author_id); ?></span></h2></div>	
		...
		<div class="white-gradient add-top-borders">	
            ...
            <p id="video-container" style="display:<?php echo $video_conatiner;?>">
             <video width="600" height="340" controls id="video_palyer" controlsList="nodownload">
   ...
   <div class="notes-list-view">	
		...
		<div class="container">
						...
						<?php if($media['type']=='image'){?>
                            ...
                            <?php }else{
								...
								if($file_type =='image'){
									...
									<?php }else{?>
									 <a download   href="<?php echo get_permalink().'/?download='.$thmbimg_url;?>">
                            ...
                            <?php }elseif($media['type']=='video' or $media['type']=='audio') {								
								...
								?> 
                            <a data-desc="<?php echo (!empty($media['desc'])) ? $media['desc'] : '';?>" data-src="<?php echo 'https://'.$ll_video_url_s3;?>" class="media-src" data-type="video" href="javascript:void(0);">
         ...
         <?php 
			if(current_user_can( 'edit_others_posts', $post->ID ) || ($post->post_author == $current_user->ID))  { 
				...
				echo '<a class="btn-gradient red-gradient" href="'.site_url('/contributor-history/').'?id='.$post->ID.'&edit=true">Edit</a>';
				echo '<p>Below is your opportunity to share your entry. </p></div>';
    ...
    jQuery(document).ready(function($) {
		...
		$('.share-box-social').click(function(e) {
        e.preventDefault();
