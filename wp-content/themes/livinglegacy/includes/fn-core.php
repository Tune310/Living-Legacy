...
function am_add_javascript( ) {
    ...
    wp_enqueue_script("fontawesome", "https://use.fontawesome.com/releases/v5.0.13/js/all.js", array(), null);
    if( !is_admin() ) {
        $am_files = array('includes/js/jquery.slicknav.js', 'includes/js/slick.min.js', 'includes/js/jquery.fancybox.min.js', 'includes/js/general.js'); // example: array('script1', 'script2');
	...
	wp_enqueue_script( 'fine-uploader', get_theme_file_uri( '/includes/js/fine-uploader.min.js' ), array('jquery'), NULL );
        ...
        $scriptData = array(
			...
			'images_dir' => get_theme_file_uri( "images/" , __FILE__),
...
function am_add_css( ) {
    ...
    // internal CSS
    $am_files = array('includes/css/slick.css', 'includes/css/slicknav.css', 'includes/css/jquery.fancybox.css', 'style.css'); 
    foreach($am_files as $am_file){
